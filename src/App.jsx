...
export default function App() {
  // Stage 5.1: pass SLUGS (not names) into ProductCatalogue
  const [presetCategorySlug, setPresetCategorySlug] = useState("");

  const handleCoreRangeClick = useCallback((categorySlug) => {
    // Persist for refresh
    try {
      localStorage.setItem("cb_category_slug", categorySlug); // ✅ correct key
      localStorage.removeItem("cb_category_name"); // ✅ remove legacy key if present
    } catch {
      ...
      <div className="cb-page cb-page--dark">
        <header className="cb-header">
          <div className="cb-header__inner">
            ...
            <div className="cb-header__search">
              <input type="search" placeholder="Search catalogue..." aria-label="Search" disabled />
        ...
        <main>
          ...
          <section id="products" className="cb-section">
            <div className="cb-section__inner">
              ...
              <p className="cb-mission__text">
                Fully structured catalogue including fixings, sealants, power tools, Paslode, drill bits, abrasives, PPE,
                ironmongery and fire rated products — ready to drive real trade enquiries.
              </p>

              <div className="cb-category-grid">
                <button type="button" className="cb-category-card" onClick={() => handleCoreRangeClick(CORE_CATEGORY_SLUGS.fixings)}>
                ...
                <button
                  ...
                  onClick={() => handleCoreRangeClick(CORE_CATEGORY_SLUGS.sealantsAdhesives)}
                >
                  <div className="cb-category-card__icon">🧴</div>
                  <div className="cb-category-card__label">Sealants &amp; Adhesives</div>
                </button>

                <button type="button" className="cb-category-card" onClick={() => handleCoreRangeClick(CORE_CATEGORY_SLUGS.powerTools)}>
                  <div className="cb-category-card__icon">⚡</div>
                  <div className="cb-category-card__label">Power Tools</div>
                </button>

                <button type="button" className="cb-category-card" onClick={() => handleCoreRangeClick(CORE_CATEGORY_SLUGS.fireRated)}>
                  <div className="cb-category-card__icon">🔥</div>
          ...
          <section id="catalogue" className="cb-section">
            <ProductCatalogue presetCategorySlug={presetCategorySlug} onPresetConsumed={handlePresetConsumed} />
          </section>
          ...
          <section id="about" className="cb-section cb-section--mission">
            <div className="cb-section__inner cb-mission">
              ...
              <p className="cb-mission__text">
                Craighead Building Supplies specialise in fixings, sealants, adhesives and fire-rated products, backed by a fully
                categorised online catalogue and dedicated Paslode repair &amp; training centre.
              </p>
            </div>
          </section>
